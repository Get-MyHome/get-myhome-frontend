...
import { useSessionRaw, writeSessionState } from "@/utils/sessionState";
...
export function Splash() {
  // 세션 기록이 있으면 아예 그리지 않는다. SSR 에서는 null 이라 첫 진입처럼 그린다
  const alreadySeen = useSessionRaw(SPLASH_SEEN_STORAGE_KEY) !== null;
  ...
  useEffect(() => {
    ...
    const toDone = setTimeout(() => {
      setPhase("done");
      // 노출이 끝난 뒤에 기록한다. 미리 쓰면 리렌더 때 스플래시가 사라진다
      writeSessionState(SPLASH_SEEN_STORAGE_KEY, true);
    }, VISIBLE_MS + FADE_MS);
